...
description: '既存コードベースのマッピング・ドキュメント化・オンボーディングをユーザーが明示的に依頼したときに使用するスキル。「このコードベースをマップして」「アーキテクチャを文書化して」「このリポジトリにオンボードして」「コードベースのドキュメントを作って」などのプロンプトで発動する。リポジトリレベルの調査を求めていない通常の機能実装・バグ修正・限定的なコード編集では発動しない。'
license: MIT
